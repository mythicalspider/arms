...
delimiter $$
create trigger if not exists after_prop_inserted
...
delimiter $$
create trigger if not exists after_observation_insert
...
after create table \.\* prop (
after ^ \*, \.\*
insert  , observation_frequency tinyint not null comment '0 means no auto observation. 10 means wait 10 seconds between observations, 20=100 secs, 30=1000 secs' 
insert  , max_severity int default 0 not null comment '0 means failure will never generate alerts, 1 means minor failure, 2 major, etc' 
